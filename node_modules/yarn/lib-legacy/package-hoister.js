...
class PackageHoister {
  ...
  seed(patterns) {
    ...
    while (true) {
      ...
      for (const _ref5 of queue) {
        ...
        const parent = _ref6[1];

        const info = this._seed(pattern, { isDirectRequire: false, parent: parent });
        if (info) {
          infos.push(info);
        }
      }
  ...
  _seed(pattern, _ref7) {
    let isDirectRequire = _ref7.isDirectRequire,
        parent = _ref7.parent;
  ...
  */
  ...
  prepass(patterns) {
    ...
    const add = (pattern, ancestry) => {
      ...
      const version = versions[pkg.version] = versions[pkg.version] || {
        occurences: new (_set || _load_set()).default(),
        pattern: pattern
      };
  ...
  init() {
    ...
    //
    for (const _ref8 of this.tree.entries()) {
