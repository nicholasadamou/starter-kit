...
class ConsoleReporter extends (_baseReporter || _load_baseReporter()).default {
  ...
  prompt(message, choices) {
    ...
    return (0, (_asyncToGenerator2 || _load_asyncToGenerator()).default)(function* () {
      ...
      let rejectRef = function rejectRef() {};
      const killListener = function killListener() {
        rejectRef();
      };

      const handleKillFromInquirer = new (_promise || _load_promise()).default(function (resolve, reject) {
        rejectRef = reject;
      });
      ...
      const answers = yield (_promise || _load_promise()).default.race([prompt([{ name: name, type: type, message: message, choices: choices, pageSize: pageSize, validate: validate }]), handleKillFromInquirer]);
